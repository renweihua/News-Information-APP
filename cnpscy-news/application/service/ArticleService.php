...
use app\common\model\Article\Articles;
use think\Validate;
...
class ArticleService extends CommonService
{
    ...
    {
        if (isset($params['category_id']) && intval($params['category_id'] ?? 0) != 0) $params['where'][] = ['category_id', '=', intval($params['category_id'] ?? 0)];
        $data_list = Articles::where([
            'is_public' => 1,
        ])->where($params['where'] ?? [])
            ->where(self::$where)
            ->with(['articleInfo'])
        ...
        if (!empty($data_list['data'])) {
            foreach ($data_list['data'] as &$value) {
                $value['article_cover'] = web_url() . $value['article_cover'];
                $value['create_date'] = date('Y-m-d', $value['create_time']);
                $value['create_date_his'] = date('Y-m-d H:i:s', $value['create_time']);
            }
        }
        return $data_list;
    }
    ...
    public static function getWebDetail($article_id = 0)
    {
        ...
        $data = $data->toArray();
        $data['article_cover'] = web_url() . $data['article_cover'];
        $data['create_date_his'] = date('Y-m-d H:i:s', $data['create_time']);
        $data['article_info']['article_content'] = htmlspecialchars_decode($data['article_info']['article_content'] ?? "");

        self::setArticleRead($article_id);
        return $data;
    }
