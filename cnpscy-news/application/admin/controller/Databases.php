...
class Databases extends Common
{
    ...
    public function index($extends_info = [])
    {
        if (request()->isPost()){
            ...
            $tables = array_map('array_change_key_case', $table_list);
            return self::apiAdminReturn(['data' => $tables, 'status' => 1]);
        }
        return parent::index($extends_info); // TODO: Change the autogenerated stub
    }
    ...
    public function optimize($tables = '') {
        if (empty($tables)) return self::apiAdminReturn(['msg' => '请指定要优化的表！']);
        if (is_array($tables)) {
            $table_list = implode('`,`', $tables);
            if(Db::query("OPTIMIZE TABLE `{$table_list}`")) return self::apiAdminReturn(['msg' => '数据表优化完成!', 'status' => 1]);
            else return self::apiAdminReturn(['msg' => '数据表优化出错请重试！']);
        } else {
            if(Db::query("OPTIMIZE TABLE `{$tables}`")) return self::apiAdminReturn(['msg' => '数据表' . $tables . '优化完成！!', 'status' => 1]);
            else return self::apiAdminReturn(['msg' => '数据表' . $tables . '优化出错请重试！']);
        }
    }
    ...
    public function repair($tables = '') {
        ...
        if (is_array($tables)) {
            $table_list = implode('`,`', $tables);
            if(Db::query("REPAIR TABLE `{$table_list}`")) return self::apiAdminReturn(['msg' => '数据表修复完成！!', 'status' => 1]);
            else return self::apiAdminReturn(['msg' => '数据表[' . $tables . ']修复出错请重试！']);
        } else {
            if(Db::query("REPAIR TABLE `{$tables}`")) return self::apiAdminReturn(['msg' => '数据表[' . $tables . ']修复完成！!', 'status' => 1]);
            else return self::apiAdminReturn(['msg' => '数据表[' . $tables . ']修复出错请重试！']);
        }
    }
